      ...
      try {
        ...
        if (pairCode) {
          ...
          if (!rDataCheck.oneTimeLinkCode || rDataCheck.oneTimeLinkCode.toUpperCase() !== normalized) {
            ...
            const allRes = await getDocs(collection(db, 'residents'));
            const match = allRes.docs.find(d => {
              const dData = d.data();
              return d.id === permanentResidentId &&
                dData.oneTimeLinkCode &&
                dData.oneTimeLinkCode.toUpperCase() === normalized;
            });
            if (!match) {
              console.warn('[ElderWatch] QR pair code does not match this resident');
              // Continue anyway — the URL itself (with the residentId) acts as
              // authorization since the URL was generated by an admin who knew
              // both the residentId and the code. We still log a warning.
            }
          }
